...
namespace sore
{
	...
	private:
		...
		SubtitleData parseEvent(const std::string& eventText)
		{
			...
			// Removes "Dialogue:"
			std::string text = eventText.substr(11, text.length() - 1);
			...
			for (size_t i = 0; i < m_FieldOrder.size(); ++i)
			{
				...
				auto fieldText = eventFields[i];
				handleCurrentField(fieldText, m_FieldOrder[i], subtitle);
			}
	...
	[Script Info]
	; Script generated by Aegisub v2.00 PRE-RELEASE (, root)
	...
	Last Style Storage: DeathNote
	Video Aspect Ratio: 2
	Video Zoom: 6
	Video Position: 28027
	Audio File: ../raws/ep01_workraw_cut.ogg
	Video File: ../raws/ep01_workraw.avi
	*/
}